...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 3. If everything oke, send token to client
...
exports.protect = catchAsync(async (req, res, next) => {
  // 1. Get token && check of it's there
  let token;
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    await user.save({ validateBeforeSave: false });
    return next(new AppError('There was and error sending the email. Please try again', 500));
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2. If token has not expired, has there is user, set te new password
  if (!user) {
